...
export default async function (orderData: iFyersSocketOrderUpdateData) {
	...
	if (orderData.status == 2) {
		if (_position.status == positionStatuses.exitPositionOrderPending || _position.status == positionStatuses.exitPositionOrderPartiallyFilled || _position.status == positionStatuses.exitPositionOrderPlaced) {
