...
export default function fetchBars() {
	...
	const OneMinuteCronJob = new cron.CronJob(
		"*/1 * * * *",
		async () => {
	...
	const FiveMinuteCronJob = new cron.CronJob(
		"*/5 * * * *",
		async () => {
			const marketOpen = await isMarketOpen()
		},
	...
	const FifteenMinuteCronJob = new cron.CronJob(
		"*/15 * * * *",
		async () => {
			const marketOpen = await isMarketOpen()
		},
	...
	const DailyCronJob = new cron.CronJob(
		"40 15 * * *",
		async () => {
			const marketOpen = await isMarketOpen()
		},
